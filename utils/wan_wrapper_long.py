...
from wan.modules.causal_model_long import CausalWanModel
...
class WanDiffusionWrapper(torch.nn.Module):
        ...
        if is_causal:
            self.model = CausalWanModel.from_pretrained(
                f"wan_models/{model_name}/", local_attn_size=local_attn_size, sink_size=sink_size, 
                    in_dim=36,                     
                    low_cpu_mem_usage=False,            # 必须为 False，以允许检查权重
                    ignore_mismatched_sizes=True        # 必须为 True，以忽略形状不匹配的错误
                    )
        else:
    ...
    def forward(
        self,
        noisy_image_or_video: torch.Tensor, y: torch.Tensor, conditional_dict: dict,
